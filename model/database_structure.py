...
class Db():
      ...
      def create_database(self):
        ...
        if created:
            ...
            self.cur.execute("ALTER TABLE admin MODIFY user_id INT AUTO_INCREMENT;")
            self.cur.execute("ALTER TABLE admin AUTO_INCREMENT = 10;")
            self.cur.execute('''CREATE TABLE if not exists event (
                             event_id INT AUTO_INCREMENT PRIMARY KEY,
                             event_name VARCHAR(100),
                             event_category ENUM('Meeting', 'Seminar', 'Training Session', 'Political Events', 'Fundraisers','Celebration','Sports Event','Webinar'),
                             address varchar(100),
                             start_date DATE,
                             end_date DATE,
                             end_time TIME,
            ...
            self.cur.execute('''CREATE TABLE if not exists survey (
                        ...
                        FOREIGN KEY (event_id) REFERENCES event(event_id));''')
            self.cur.execute('''CREATE TABLE if not exists feedbackResponse (
      ...
      def insert_dummy_data(self):
        ...
        # Insert dummy data into admin table
          self.cur.execute("INSERT INTO admin (email, password, first_name, last_name) VALUES ('jane.smith@example.com', 'password456', 'Jane', 'Smith');")
          self.cur.execute("INSERT INTO admin (email, password, first_name, last_name) VALUES ('james.smith@example.com', 'password456', 'James', 'Smith');")
...
#Insert dummy data into event table // event_categoryv datatype enum karna haii-- done
    ...
    # # Insert dummy data into sponsor table 
          ...
          self.cur.execute("INSERT INTO admin (email, password, first_name, last_name) VALUES ('john.doe@example.com', 'password123', 'John', 'Doe');")
    
    # # Insert dummy data into registration table   # ORGANISATION NAME ADD phone no and delete admin id---done
          self.cur.execute("INSERT INTO admin (email, password, first_name, last_name) VALUES ('joo.smith@example.com', 'password456', 'Joo', 'Smith');")
